...
#ahora obtenes el umbral de la imagen original para poder restarla
...
#hacemos la operacion correspondiente al umbral
for i in range(resta.shape[0]):
    for j in range(resta.shape[1]):
        if resta[i,j]<umbral:
            ...
            resta[i,j] = 255
#al final solo lo mostramos
cv2.imshow("umbral", resta)
cv2.waitKey(0)
cv2.destroyAllWindows()
